(function () {
  ...
  function parseISODateArray(isoStr) {
    if (!isoStr) return [];
    return isoStr
      .split(",")
  ...
  function renderCalendar(container) {
    ...
    if (isNaN(currentMonth) || isNaN(currentYear)) {
      ...
      const selectedDateAttr = container.getAttribute(
        "data-tui-calendar-selected-date",
      );

      if (selectedDateAttr) {
        const firstSelected = parseISODateArray(selectedDateAttr)[0];
        if (firstSelected) {
          currentMonth = firstSelected.getUTCMonth();
          currentYear = firstSelected.getUTCFullYear();
        }
      }
    ...
    const selectedDates = selectedDateStr
      ? parseISODateArray(selectedDateStr)
      : [];
  ...
  document.addEventListener("click", (e) => {
    ...
    if (e.target.matches("[data-tui-calendar-day]")) {
      ...
      const multiple =
        container.getAttribute("data-tui-calendar-multiple") === "true";
      ...
      const clickedDateISO = clickedDate.toISOString().split("T")[0];
      ...
      if (multiple) {
        ...
        let selectedDates = selectedDateStr.split(",").filter((d) => d); // array of ISO strings

        const dateIndex = selectedDates.indexOf(clickedDateISO);
        ...
        if (dateIndex > -1) {
          ...
          selectedDates.push(clickedDateISO);
        }
        newSelectedISO = selectedDates.join(",");
      } else {
        newSelectedISO = clickedDateISO;
      }
      ...
      if (hiddenInput) {
        hiddenInput.value = newSelectedISO;
        hiddenInput.dispatchEvent(new Event("change", { bubbles: true }));
      }

      const detail = {
        date: clickedDate, // The date that was just clicked
        // The full list of selected dates
        dates: newSelectedISO
          .split(",")
          .filter((d) => d)
          .map((s) => parseISODate(s)),
      };
      console.log("calendar-date-selected dispatched:", detail);
